 ...
 * @brief   SVPWM 七段式调制: 扇区判断 + 矢量时间计算 + CCR 输出
 */
#include "headline.h"
...
void svpwm_Cale(p_SVPWM pV){
    ...
    pV->tmp3= pV->tmp2 - pV->tmp1;   // Vref3 = (√3/2)Ualpha - 0.5*Ubeta
    pV->VecSector=3;                  // 基准值
    ...
    if(pV->VecSector==1 || pV->VecSector==4)   // 基本矢量扇区，计算矢量占空比 Tabc
    {
        pV->Ta= pV->tmp2;
